...
class FPSMonitor(Monitor):
    ...
    def get_FPS_info(self):
        ...
        """
        surface_list_info = self.dev.raw_shell("dumpsys SurfaceFlinger --list").decode()
        if self.dev.sdkversion >= 24:
        ...
        """
        surface_info = self.dev.raw_shell(
    ...
    def get_FPS_surface_view(self, surface_info):
        for i in surface_info.split(os.linesep):
            if "SurfaceView" not in i or "Background for" in i or self.dev.package not in i:
                continue
    ...
    def get_surfaceview_activity(self):
        ...
        activity_line = ''
        try:
            dumpsys_result = self.dev.raw_shell(
                'dumpsys SurfaceFlinger --list | {} {}'.format("grep", self.dev.package)).decode()
            dumpsys_result_list = dumpsys_result.split('\n')
            for line in dumpsys_result_list:
                if line.startswith('SurfaceView') and line.find(self.dev.package) != -1:
            ...
            if activity_line:
                ...
                if not activity_name.__contains__(self.dev.package):
                    print(
    ...
    def run(self):
        self.dev.adb.raw_shell("dumpsys SurfaceFlinger --latency-clear").decode()
        '''
        按照指定频率，循环搜集FPS的信息
        '''
        # ftimege100:增量耗时
        ...
        while G.stop_event.is_set():  # 停止了循环会停止
            ...
            try:
                ...
                print("an exception hanpend in FPS thread , reason unkown!, e:")
